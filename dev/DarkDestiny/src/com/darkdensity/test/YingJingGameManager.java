...
import com.darkdensity.core.GridManager;
import com.darkdensity.core.Player;
...
public class YingJingGameManager extends JFrame {
...
//	private InputManager inputManager;
	...
	public YingJingGameManager(int gameMode) throws IOException{
		...
		//sprite
		initSprite();
		testSpritePanel = new TestSpritePanel(this);
		add(testSpritePanel);
	...
	public void gameLoop() throws InterruptedException {
		...
		while (isRunning) {
			...
			if(step <200){
				step++;
				sprite.walkStep(sprite.FACE_EAST);
				}
			else if(200 <= step && step<400 ){
				step++;
				sprite.walkStep(sprite.FACE_SOUTH);
			}
			else if(400 <= step && step<1000 ){
				step++;
				sprite.walkStep(sprite.FACE_EAST);
			}
			else if(1000 <= step && step<1400 ){
				step++;
				sprite.walkStep(sprite.FACE_SOUTH_WEST);
			}
			if(step ==1400){
				step++;
				sprite.stop();
			...
			public void keyPressed(KeyEvent e) {
				...
				if (key == KeyEvent.VK_ENTER
					...
					if (dialogresult == JOptionPane.YES_OPTION) {
						...
						new Thread(r).start();
						YingJingGameManager.this.dispose();
					}
				}
			}
		});
	...
	/**
	 * @Title: checkControlChange
	 * @Description: this method is not use by now
	 */
	public void checkControlChange() {
		// check whether the mouse has touch the screen border
		// point where the render begins
		int iRenderX = map.iRenderX;
		int iRenderY = map.iRenderY;
		int SCREEN_MOVE_SPEED = 8;

		if (controlManager.getMouseX() == 0 && iRenderX > 0) {
			iRenderX = iRenderX - SCREEN_MOVE_SPEED;
		}
		if (controlManager.getMouseY() == 0 && iRenderY > 0) {
			iRenderY = iRenderY - SCREEN_MOVE_SPEED;
		}
		if (controlManager.getMouseX() == Config.PANEL_WIDTH - 1
				&& iRenderX < (map.mapWidthPx - Config.PANEL_WIDTH)) {
			iRenderX = iRenderX + SCREEN_MOVE_SPEED;
		}
		if (controlManager.getMouseY() == Config.PANEL_HEIGHT - 1
				&& iRenderY < (map.mapHeightPx - Config.PANEL_HEIGHT)) {
			iRenderY = iRenderY + SCREEN_MOVE_SPEED;
		}
		map.iRenderX = iRenderX;
